...
class Trader:
    ...
    def generate_signal(self, state: TradingState, sunlightList, humidityList) -> Tuple[float, float]:
        ...
        if len(sunlightList) > self.signal_window and len(humidityList) > self.signal_window:
            ...
            min_sunlight = min(sunlightList)
            max_sunlight = max(sunlightList)
            min_humidity = min(humidityList)
            max_humidity = max(humidityList)
            ...
            if sunshine_std_dev != 0 and humidity_std_dev != 0:
                ...
                prev_sunshine_norm = (prev_sunshine - min_sunlight) / (max_sunlight - min_sunlight)
                prev_humidity_norm = (prev_humidity - min_humidity) / (max_humidity - min_humidity)
