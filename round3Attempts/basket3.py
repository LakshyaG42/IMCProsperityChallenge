    ...
    def compute_orders_all_products(self, order_depth):
        ...
        for product in ['CHOCOLATE', 'STRAWBERRIES', 'ROSES']:
            if int(round(self.position[product])) > 0:
                val_ord = self.position_limit[product] - self.position[product]
                if val_ord > 0:
                    orders[product].append(Order(product, worst_sell[product], val_ord))
            elif int(round(self.position[product])) < 0:
                val_ord = -(self.position_limit[product] + self.position[product])
                if val_ord < 0:
                    orders[product].append(Order(product, worst_buy[product], val_ord))
