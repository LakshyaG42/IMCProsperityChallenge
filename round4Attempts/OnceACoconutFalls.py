...
class Trader:
    ...
    def calculate_coconut_momentum_signal(self, state: TradingState) -> Tuple[float, int]:
        coconut_momentum = 0
        coconut_coupon_momentum = 0
    
        if len(self.coconutPrices) > 6 and len(self.coconutCouponPrices) > 6:
            coconut_momentum = self.coconutPrices[-1] - self.coconutPrices[-5]
            coconut_coupon_momentum = self.coconutCouponPrices[-1] - self.coconutCouponPrices[-5]
        return coconut_momentum, coconut_coupon_momentum
    ...
    def run(self, state: TradingState):
        result = {}
        best_buy_price, bestest_bid_amount = list(state.order_depths['COCONUT'].buy_orders.items())[0]
        best_sell_price, bestest_ask_amount = list(state.order_depths['COCONUT'].sell_orders.items())[0]
        coconut_mid_price = (best_sell_price + best_buy_price) / 2
        self.coconutPrices.append(coconut_mid_price)
        best_buy_price1, bestest_bid_amount = list(state.order_depths['COCONUT_COUPON'].buy_orders.items())[0]
        best_sell_price1, bestest_ask_amount = list(state.order_depths['COCONUT_COUPON'].sell_orders.items())[0]
        coupon_mid_price = (best_sell_price1 + best_buy_price1) / 2
        self.coconutCouponPrices.append(coupon_mid_price)
        ...
        for product in state.order_depths:
            ...
            if(product in ('COCONUT', 'COCONUT_COUPON')):
                    coconut_momentum, coconut_coupon_momentum = self.calculate_coconut_momentum_signal(state)
                    print(f"   CoconutPrices: {self.coconutPrices} CouponPrices: {self.coconutCouponPrices}  Coconut momentum: {coconut_momentum}, Coconut Coupon momentum: {coconut_coupon_momentum}    ")
                    if(product == 'COCONUT'):
                        if coconut_momentum > 5:
                            if len(order_depth['COCONUT'].sell_orders) != 0:
                                best_ask, best_ask_amount = list(order_depth['COCONUT'].sell_orders.items())[0]
                                allowable_quantity = self.calculate_allowable_quantity('COCONUT', 'BUY', best_ask_amount)
                                orders.append(Order('COCONUT', best_ask, best_ask_amount))
                                self.update_position('COCONUT', 'BUY', allowable_quantity)
                        elif coconut_momentum < -5:
                            if len(order_depth['COCONUT_COUPON'].buy_orders) != 0:
                                best_bid, best_bid_amount = list(order_depth['COCONUT_COUPON'].buy_orders.items())[0]
                                allowable_quantity = self.calculate_allowable_quantity('COCONUT_COUPON', 'SELL', best_bid_amount)
                                orders.append(Order('COCONUT_COUPON', best_bid, -best_bid_amount))
                                self.update_position('COCONUT_COUPON', 'SELL', allowable_quantity)
                    if(product == 'COCONUT_COUPON'):
                        if coconut_coupon_momentum > 5:
                            if len(order_depth['COCONUT_COUPON'].sell_orders) != 0:
                                best_ask, best_ask_amount = list(order_depth['COCONUT_COUPON'].sell_orders.items())[0]
                                allowable_quantity = self.calculate_allowable_quantity('COCONUT_COUPON', 'BUY', best_ask_amount)
                                orders.append(Order('COCONUT_COUPON', best_ask, best_ask_amount))
                                self.update_position('COCONUT_COUPON', 'BUY', allowable_quantity)
                        elif coconut_coupon_momentum < -5:
                            if len(order_depth['COCONUT'].buy_orders) != 0:
                                best_bid, best_bid_amount = list(order_depth['COCONUT'].buy_orders.items())[0]
                                allowable_quantity = self.calculate_allowable_quantity('COCONUT', 'SELL', best_bid_amount)
                                orders.append(Order('COCONUT', best_bid, -best_bid_amount))
                                self.update_position('COCONUT', 'SELL', allowable_quantity)
                    # if coconut_momentum > 0 and coconut_coupon_momentum < 0:
                    #     if len(order_depth['COCONUT'].sell_orders) != 0:
                    #         best_ask, best_ask_amount = list(order_depth['COCONUT'].sell_orders.items())[0]
                    #         allowable_quantity = self.calculate_allowable_quantity('COCONUT', 'BUY', best_ask_amount)
                    #         orders.append(Order('COCONUT', best_ask, best_ask_amount))
                    ...
                    #         orders.append(Order('COCONUT_COUPON', best_bid, -best_bid_amount))
                    #         self.update_position('COCONUT_COUPON', 'SELL', allowable_quantity)
                    # elif coconut_momentum < 0 and coconut_coupon_momentum > 0:
                    #     if len(order_depth['COCONUT'].buy_orders) != 0:
                    #         best_bid, best_bid_amount = list(order_depth['COCONUT'].buy_orders.items())[0]
                    #         allowable_quantity = self.calculate_allowable_quantity('COCONUT', 'SELL', best_bid_amount)
                    #         orders.append(Order('COCONUT', best_bid, -best_bid_amount))
