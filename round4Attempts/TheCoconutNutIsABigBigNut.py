...
class Trader:
    ...
    def pairs_trade_coconuts_coupons(self, state: TradingState):
                ...
                # Calculate the spread (difference) between Coconut and Coconut Coupon prices
                coconut_price = mid_prices['COCONUT']
                coupon_price = mid_prices['COCONUT_COUPON']
                spread = coconut_price - coupon_price

                # Define entry and exit thresholds for the spread
                entry_threshold = 9000 
                exit_threshold = 9317

                # Check if the spread exceeds the entry threshold for going long Coconut Coupons and short Coconuts
                if spread > entry_threshold:
                    coconut_qty = self.calculate_allowable_quantity('COCONUT', 'SELL', self.position['COCONUT'])
                    coupon_qty = self.calculate_allowable_quantity('COCONUT_COUPON', 'BUY', self.position['COCONUT_COUPON'])
                    
                    # Place orders to trade Coconut Coupons and Coconuts
                    if coconut_qty > 0:
                        orders.append(Order('COCONUT', coconut_price, -coconut_qty))
                        self.update_position('COCONUT', 'SELL', coconut_qty)
                    if coupon_qty > 0:
                        orders.append(Order('COCONUT_COUPON', coupon_price, coupon_qty))
                        self.update_position('COCONUT_COUPON', 'BUY', coupon_qty)

                # Check if the spread falls below the exit threshold for closing the position
                elif spread < -exit_threshold:
                    coupon_qty = self.calculate_allowable_quantity('COCONUT_COUPON', 'SELL', self.position['COCONUT_COUPON'])
                    coconut_qty = self.calculate_allowable_quantity('COCONUT', 'BUY', self.position['COCONUT'])
                    
                    # Place orders to close the position
                    if coupon_qty > 0:
                        orders.append(Order('COCONUT_COUPON', coupon_price, -coupon_qty))
                        self.update_position('COCONUT_COUPON', 'SELL', coupon_qty)
                    if coconut_qty > 0:
                        orders.append(Order('COCONUT', coconut_price, coconut_qty))
                        self.update_position('COCONUT', 'BUY', coconut_qty)
        ...
        for product in state.order_depths:
            ...
            if(product in ('CHOCOLATE', 'STRAWBERRIES', 'ROSES', 'GIFT_BASKET')):
                ...
                if(product in ('COCONUT', 'COCONUT_COUPON')):
                    orders += self.pairs_trade_coconuts_coupons(state)[product]
